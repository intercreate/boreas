...
void shell_completion(struct shell *sh)
{
	...
	bool completing_partial =
		(sh->ctx.cmd_buff_pos > 0 && sh->ctx.cmd_buff[sh->ctx.cmd_buff_pos - 1] != ' ');
	...
	for (size_t i = 0; i < candidate_count; i++) {
		...
		if (strncmp(syntax, partial, partial_len) == 0) {
			...
			if (match_count >= CONFIG_ZSHELL_MAX_ROOT_CMDS) {
				break;
			}
		}
	}
	...
	for (size_t i = 1; i < match_count; i++) {
		size_t j = 0;
		while (j < common_len && matches[0]->syntax[j] == matches[i]->syntax[j]) {
			j++;
		}
		common_len = j;
	}
