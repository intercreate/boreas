...
{
	const char *prompt = sh->default_prompt ? sh->default_prompt : CONFIG_ZSHELL_PROMPT;
...
static void shell_process_char(struct shell *sh, char ch)
{
	...
	case '\n':
		...
		if (sh->ctx.cmd_buff_len > 0) {
			...
			shell_history_add(&sh->history, sh->ctx.cmd_buff, sh->ctx.cmd_buff_len);
	...
	case 0x17: /* Ctrl-W: delete word backward */
		if (sh->ctx.cmd_buff_pos > 0) {
			...
			memmove(&sh->ctx.cmd_buff[sh->ctx.cmd_buff_pos], &sh->ctx.cmd_buff[start],
...
int shell_init(struct shell *sh, const struct shell_transport *transport, const char *prompt)
{
...
#if !defined(CONFIG_IDF_TARGET_LINUX)
	{
		...
		const size_t n = (size_t)(_shell_root_cmds_end - _shell_root_cmds_start);
		for (size_t i = 0; i < n; i++) {
			shell_cmd_register(&_shell_root_cmds_start[i]);
		}
	}
#endif
	...
	k_thread_create(&sh->thread, shell_stack, CONFIG_ZSHELL_THREAD_STACK_SIZE,
			shell_thread_entry, sh, NULL, NULL, CONFIG_ZSHELL_THREAD_PRIORITY, 0,
			K_NO_WAIT);
