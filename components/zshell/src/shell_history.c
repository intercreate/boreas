...
void shell_history_add(struct shell_history *hist, const char *line, uint16_t len)
{
	...
	if (!sys_dlist_is_empty(&hist->list)) {
		struct shell_history_entry *tail = CONTAINER_OF(sys_dlist_peek_tail(&hist->list),
	...
	uint16_t copy_len =
		len < CONFIG_ZSHELL_CMD_BUFF_SIZE - 1 ? len : CONFIG_ZSHELL_CMD_BUFF_SIZE - 1;
...
bool shell_history_get(struct shell_history *hist, bool up, char *dst, uint16_t *len)
{
	...
	if (up) {
		...
		if (hist->current == NULL) {
			...
			sys_dnode_t *prev = sys_dlist_peek_prev(&hist->list, hist->current);
			if (prev != NULL) {
				hist->current = prev;
			}
