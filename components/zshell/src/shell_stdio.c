...
static int shell_stdio_init(const struct shell_transport *transport, void *context)
{
...
static int shell_stdio_write(const struct shell_transport *transport, const void *data,
...
static int shell_stdio_read(const struct shell_transport *transport, void *data, size_t length,
			    size_t *cnt)
{
	if (!stdio_ctx.initialized) {
		*cnt = 0;
