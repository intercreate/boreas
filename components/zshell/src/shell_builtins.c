	...
	if (argc > 1) {
		...
		for (size_t i = 0; i < _shell_root_cmd_count; i++) {
			if (strcmp(_shell_root_cmds[i]->syntax, argv[1]) == 0) {
				...
				shell_print(sh, "%s - %s", cmd->syntax, cmd->help ? cmd->help : "");
...
static int cmd_kernel_heap(const struct shell *sh, size_t argc, char **argv)
{
	(void)argc;
	(void)argv;
	shell_print(sh, "Free heap:    %lu bytes", (unsigned long)esp_get_free_heap_size());
	shell_print(sh, "Min free:     %lu bytes", (unsigned long)esp_get_minimum_free_heap_size());
	return 0;
}

SHELL_STATIC_SUBCMD_SET_CREATE(kernel_subcmds,
			       SHELL_CMD(uptime, NULL, "Print system uptime", cmd_kernel_uptime)
				       SHELL_CMD(heap, NULL, "Print heap statistics",
						 cmd_kernel_heap));
