...
SHELL_STATIC_SUBCMD_SET_CREATE(log_subcmds,
			       SHELL_CMD(list, NULL, "List registered log modules", cmd_log_list)
				       SHELL_CMD(level, NULL,
						 "Set log level: log level <module> <0-4>",
						 cmd_log_level));
...
static int cmd_thread(const struct shell *sh, size_t argc, char **argv)
{
	...
	shell_print(sh, "%-20s %6s %6s  %-8s %4s", "Name", "Stack", "HWM", "State", "Prio");
	...
	for (UBaseType_t i = 0; i < actual; i++) {
		...
		shell_print(sh, "%-20s %6s %6lu  %-8s %4lu", t->pcTaskName, "?",
			    (unsigned long)hwm_bytes, state_str(t->eCurrentState),
