 ...
 */
static const struct shell_static_entry *find_subcmd(const union shell_cmd_entry *subcmd,
						    const char *name)
{
...
int shell_execute(struct shell *sh)
{
	...
	while (cmd_depth < argc && cmd->subcmd != NULL) {
		const struct shell_static_entry *sub = find_subcmd(cmd->subcmd, argv[cmd_depth]);
		if (sub == NULL) {
