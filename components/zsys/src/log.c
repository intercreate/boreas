...
void zsys_log_list_modules(void)
{
	...
	ESP_LOGI(TAG, "  %-24s  %s", "------------------------", "-----");
	for (int i = 0; i < module_count; i++) {
		ESP_LOGI(TAG, "  %-24s  %s", modules[i].name,
...
void zsys_log_backend_register(const struct log_backend *backend)
{
	if (_log_backend_count >= CONFIG_ZSYS_LOG_MAX_BACKENDS) {
		ESP_LOGW(TAG, "Backend registry full, cannot register '%s'", backend->name);
		return;
	}
...
K_MSGQ_DEFINE(_zsys_log_msgq, sizeof(struct log_msg), CONFIG_ZSYS_LOG_BUFFER_COUNT, 4);

K_THREAD_STACK_DEFINE(_log_thread_stack, CONFIG_ZSYS_LOG_THREAD_STACK_SIZE);
...
static void fill_log_msg(struct log_msg *msg, uint8_t level, const char *module, const char *fmt,
			 va_list args)
{
	...
	if (xPortInIsrContext()) {
		...
		if (task) {
			strncpy(msg->thread, pcTaskGetName(task), ZSYS_LOG_THREAD_NAME_MAX - 1);
...
int zsys_log_init(void)
{
	...
	{
		const size_t n = (size_t)(_log_module_entries_end - _log_module_entries_start);
...
#if defined(CONFIG_ZSYS_LOG_MODE_DEFERRED)
	k_msgq_init(&_zsys_log_msgq, (char *)_zsys_log_msgq.storage, sizeof(struct log_msg),
		    CONFIG_ZSYS_LOG_BUFFER_COUNT);

	_log_mode = LOG_MODE_DEFERRED;

	_log_thread.name = "zsys_log";
	k_thread_create(&_log_thread, _log_thread_stack, CONFIG_ZSYS_LOG_THREAD_STACK_SIZE,
			log_output_thread, NULL, NULL, NULL, CONFIG_ZSYS_LOG_THREAD_PRIORITY, 0,
			K_NO_WAIT);
	ESP_LOGI(TAG, "Deferred logging started (queue=%d, stack=%d)", CONFIG_ZSYS_LOG_BUFFER_COUNT,
		 CONFIG_ZSYS_LOG_THREAD_STACK_SIZE);
#else
	ESP_LOGI(TAG, "Synchronous logging initialized (%d backends)", _log_backend_count);
#endif
		      ...
		      const char *label)
{
	...
	zsys_log_msg_emit(level, module, "%s (%u bytes):", label ? label : "hex", (unsigned)len);
