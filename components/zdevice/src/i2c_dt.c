...
esp_err_t i2c_write_read_dt(const struct i2c_dt_spec *spec, const uint8_t *write_buf,
			    size_t num_write, uint8_t *read_buf, size_t num_read)
{
	i2c_master_dev_handle_t handle = i2c_dt_resolve(spec);
	if (handle == NULL) {
		return ESP_ERR_INVALID_STATE;
	}
	return i2c_master_transmit_receive(handle, write_buf, num_write, read_buf, num_read,
					   I2C_DT_TIMEOUT_MS);
}
...
esp_err_t i2c_burst_write_dt(const struct i2c_dt_spec *spec, uint8_t reg, const uint8_t *buf,
			     size_t len)
{
