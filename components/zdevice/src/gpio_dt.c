...
static esp_err_t gpio_esp32_pin_interrupt_configure(const struct device *port, gpio_num_t pin,
						    uint32_t flags)
{
	(void)port;
...
static esp_err_t gpio_esp32_manage_callback(const struct device *port, struct gpio_callback *cb,
					    bool set)
{
	(void)port;
