...
static void sort_entries(const struct sys_init_entry **sorted, size_t count)
{
	for (size_t i = 1; i < count; i++) {
		...
		while (j >= 0 &&
			...
			(sorted[j]->level == key->level && sorted[j]->priority > key->priority))) {
			sorted[j + 1] = sorted[j];
			j--;
		}
		sorted[j + 1] = key;
...
int sys_init_run_all(void)
{
	...
	if (count > CONFIG_ZKERNEL_SYS_INIT_MAX_ENTRIES) {
		ESP_LOGE(TAG, "Too many init entries (%u > %d)", (unsigned)count,
			 CONFIG_ZKERNEL_SYS_INIT_MAX_ENTRIES);
	...
	for (size_t i = 0; i < count; i++) {
		...
		ESP_LOGI(TAG, "[%d/%d] %s (level=%d, prio=%d)", (int)(i + 1), (int)count,
			 entry->name, entry->level, entry->priority);
