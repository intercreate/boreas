...
int k_msgq_init(struct k_msgq *msgq, char *buffer, size_t msg_size, uint32_t max_msgs)
{
	...
	msgq->handle = xQueueCreateStatic(max_msgs, msg_size, msgq->storage, &msgq->buffer);
	if (msgq->handle == NULL) {
...
int k_msgq_put(struct k_msgq *msgq, const void *data, k_timeout_t timeout)
{
	...
	if (xPortInIsrContext()) {
		...
		ret = xQueueSendToBack(msgq->handle, data, k_timeout_to_ticks(timeout));
	}
	if (ret == pdTRUE) {
		return 0;
	}
...
int k_msgq_get(struct k_msgq *msgq, void *data, k_timeout_t timeout)
{
	...
	if (xPortInIsrContext()) {
		...
		ret = xQueueReceive(msgq->handle, data, k_timeout_to_ticks(timeout));
	}
	if (ret == pdTRUE) {
		return 0;
	}
