...
static inline void sys_slist_remove(sys_slist_t *list, sys_snode_t *prev, sys_snode_t *node)
{
	if (prev == NULL) {
		list->head = node->next;
	} else {
...
#define SYS_SLIST_FOR_EACH_NODE(list, node)                                                        \
	for ((node) = sys_slist_peek_head(list); (node) != NULL; (node) = sys_slist_peek_next(node))
...
#define SYS_SLIST_FOR_EACH_NODE_SAFE(list, node, next_node)                                        \
	for ((node) = sys_slist_peek_head(list),                                                   \
	    (next_node) = (node) ? sys_slist_peek_next(node) : NULL;                               \
	     (node) != NULL;                                                                       \
	     (node) = (next_node), (next_node) = (node) ? sys_slist_peek_next(node) : NULL)
...
#define SYS_SLIST_PEEK_NEXT_CONTAINER(container, field)                                            \
	SYS_SLIST_CONTAINER(sys_slist_peek_next(&(container)->field), container, field)

#define SYS_SLIST_FOR_EACH_CONTAINER(list, container, field)                                       \
	for ((container) = SYS_SLIST_PEEK_HEAD_CONTAINER(list, container, field);                  \
	     (container) != NULL; (container) = SYS_SLIST_PEEK_NEXT_CONTAINER(container, field))

#define SYS_SLIST_FOR_EACH_CONTAINER_SAFE(list, container, tmp, field)                             \
	for ((container) = SYS_SLIST_PEEK_HEAD_CONTAINER(list, container, field),                  \
	    (tmp) = (container) ? SYS_SLIST_PEEK_NEXT_CONTAINER(container, field) : NULL;          \
