...
static void test_mutex_priority_boost(void)
{
    ...
    k_thread_create(&low_thread, low_stack, K_THREAD_STACK_SIZEOF(low_stack),
                    low_prio_holder, &pi_mutex, NULL, NULL,
    ...
    k_thread_create(&high_thread, high_stack, K_THREAD_STACK_SIZEOF(high_stack),
                    high_prio_waiter, &pi_mutex, NULL, NULL,
...
static void test_mutex_priority_restore(void)
{
    ...
    k_thread_create(&low_thread, low_stack, K_THREAD_STACK_SIZEOF(low_stack),
                    low_prio_check_restore, &pi_mutex, NULL, NULL,
                    PRIO_LOW, 0, K_NO_WAIT);
...
static void test_mutex_pi_prevents_inversion(void)
{
    ...
    /* 1. Low grabs mutex */
    k_thread_create(&pi3_low_thread, pi3_low_stack,
                    K_THREAD_STACK_SIZEOF(pi3_low_stack),
                    low_prio_timed, &pi_mutex, NULL, NULL,
                    PRIO_LOW, 0, K_NO_WAIT);

    k_msleep(10); /* let low grab the mutex */

    /* 2. High tries to take mutex -- blocks, triggers PI boost on low */
    k_thread_create(&pi3_high_thread, pi3_high_stack,
                    K_THREAD_STACK_SIZEOF(pi3_high_stack),
                    high_prio_timed, &pi_mutex, NULL, NULL,
                    PRIO_HIGH, 0, K_NO_WAIT);

    /* 3. Medium becomes runnable -- without PI, it would preempt low */
    k_thread_create(&pi3_med_thread, pi3_med_stack,
                    K_THREAD_STACK_SIZEOF(pi3_med_stack),
                    medium_prio_worker, NULL, NULL, NULL,
                    PRIO_MEDIUM, 0, K_NO_WAIT);
