...
static void async_cb(const struct device *dev, struct uart_event *evt, void *user_data)
{
	...
	case UART_TX_ABORTED:
		...
		break;
	case UART_RX_RDY:
		on_rx_chunk(evt->data.rx.buf + evt->data.rx.offset, evt->data.rx.len);
		break;
	...
	case UART_RX_STOPPED:
		LOG_ERR("RX stopped, reason=0x%x", (unsigned)evt->data.rx_stop.reason);
...
void app_main(void)
{
	...
	LOG_INF("Listening on UART%d: tx=%d rx=%d DE=%d baud=%d", UART_NUM_1, UART_TX_PIN,
		UART_RX_PIN, DE_PIN, 115200);
