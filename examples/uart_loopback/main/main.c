...
DEVICE_DEFINE(uart1, uart_esp32_init, &uart_esp32_api, &uart1_config, &uart1_data, NULL);
...
static void irq_cb(const struct device *dev, void *user_data)
{
	...
	if (uart_irq_tx_ready(dev) && irq_tx_idx < sizeof(irq_tx) - 1) {
		...
		int n = uart_fifo_fill(dev, (const uint8_t *)&irq_tx[irq_tx_idx], remaining);
		irq_tx_idx += (size_t)n;
...
static void async_cb(const struct device *dev, struct uart_event *evt, void *user_data)
{
	(void)dev;
	(void)user_data;
...
static bool phase_async(void)
{
	...
	LOG_INF("phase3 async rx='%.*s' total=%u (tx_rc=%d rx_rc=%d)", (int)async_rx_total,
		(char *)async_rx_buf, (unsigned)async_rx_total, rc_tx, rc_rx);

	return rc_tx == 0 && rc_rx == 0 && async_rx_total == expect &&
...
static void idle_cb(const struct device *dev, struct uart_event *evt, void *user_data)
{
	(void)dev;
	(void)user_data;
	...
	case UART_RX_DISABLED:
		...
		break;
	case UART_RX_STOPPED:
		LOG_WRN("phase4 rx stopped, reason=0x%x", (unsigned)evt->data.rx_stop.reason);
		break;
	case UART_TX_ABORTED:
		k_sem_give(&idle_tx_done);
		break;
...
static bool phase_idle_timeout(void)
{
	...
	if (uart_rx_enable(&uart1, idle_rx_buf, sizeof(idle_rx_buf), PHASE4_TIMEOUT_US) != 0) {
		LOG_ERR("rx_enable failed");
		return false;
	}
	...
	LOG_INF("phase4 idle-timeout rx='%.*s' total=%u "
		...
		(int)idle_rx_total, (char *)idle_rx_buf, (unsigned)idle_rx_total, rc_tx, rc_rx,
		(int)idle_rx_ready_seen);

	return rc_tx == 0 && rc_rx == 0 && idle_rx_ready_seen && idle_rx_total == expect &&
